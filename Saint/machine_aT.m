...
uitable('Parent', tab3,'Units','normal','Position',[0.14 0.6 0.34  0.3],'Visible','on','Tag','machine_data_tab_1','Data',dat,'ColumnName',var1 ); %machine 1 preview  table
...
pan_1 = uipanel('Parent',tab3,'Tag','datapres_1','Position',[0.03 0.1 0.45 0.4],'Visible','off'); % machine 1 preview figure
...
uitable('Parent', tab3,'Units','normal','Position',[0.63 0.6 0.34  0.3],'Visible','on','Tag','machine_data_tab_2','Data',dat,'ColumnName',var1); %machine 2 preview  table
...
%
c='!!!!!!! HELP !!!!!!';
message=["Clear the space and type the new machine's name. Common machine names are:    1. TRAINERS 2. SPIKE" ];
...
if strcmp(tag1(end),'2') % new info button
    ...
    messag=["Clear the space and type the new machine's name. Common machine names are:    1. TRAINERS 2. SPIKE" ];
...
if strcmp(tag1(end-2),'1')
    ...
    assignin('base',strcat('dat_machine1_',tag1(end)),machine_i.Data);
    ...
    assignin('base',strcat('dat_machine_',tag1(end)),machine_i.Data);
end
...
assignin('base','SecondMachine',[list_21{sel_var_2} ' OF ' upper(sel_machine_2) ' OF ' upper(sel_surf_2) ' OF ' upper(sel_sub_2)]);
...
uicontrol('Parent', tab4, 'Style', 'pushbutton', 'String', FirstMachine,'Tag','machine_data_prev1_1', ...
    'HorizontalAlignment', 'left', 'Units','normal','Position', [0.02 0.92 0.45 0.04]);%machine 1 variable  preview of peaks button

uicontrol('Parent', tab4, 'Style', 'pushbutton', 'String', 'Move up','Tag','variables1_2', ...
...
uicontrol('Parent', tab4, 'Style', 'pushbutton', 'String', 'Move down','Tag','variables1_3', ...
    'HorizontalAlignment', 'left', 'Units','normal','BackgroundColor', 'green','Position', [0.02 0.83 0.1 0.03],'Callback',@movelims);

% new
uicontrol('Parent', tab4, 'Style', 'pushbutton', 'String', 'Move up (+)','Tag','variables1_v2', ...
...
uicontrol('Parent', tab4, 'Style', 'pushbutton', 'String', 'Move down (-)','Tag','variables1_v3', ...
    'HorizontalAlignment', 'left', 'Units','normal','BackgroundColor', 'green','Position', [0.02 0.69 0.1 0.03],'Callback',@movelims2);
%new end


uicontrol('Parent', tab4, 'Style', 'pushbutton', 'String', 'Update','Tag','variables1_4', ...
...
uicontrol('Parent', tab4,'Style', 'popup','String', list_6 ,'Tag','variables1_13',...
    'Units','normal','Visible','off','Position', [0.02 0.3 0.1 0.04],'BackgroundColor', 'green','Callback', @selectpeakareas); %machine 2 variable  selection of spikes of peaks pop-up
...
pan_1 = uipanel('Parent',tab4,'Tag','datapre1_1','Position',[0.13 0.16 0.34 0.37],'Visible','off'); % machine 1 preview figure
pan1_1 = uipanel('Parent',tab4,'Tag','datapre1_2','Position',[0.13 0.54 0.34 0.37],'Visible','on'); % machine 1 preview figure
subplot(1,1,1,'Parent', pan_1) ;
plot(dat(:,2))
[w,~] = size(dat);
xlim([0 1.1*w])
...
uicontrol('Parent', tab4, 'Style', 'pushbutton', 'String', SecondMachine,'Tag','machine_data_prev2_1', ...
    'HorizontalAlignment', 'left', 'Units','normal','Position', [0.51 0.92 0.45 0.04]); %machine 2 variable  preview of peaks button

uicontrol('Parent', tab4, 'Style', 'pushbutton', 'String', 'Move up','Tag','variables2_2', ...
...
uicontrol('Parent', tab4, 'Style', 'pushbutton', 'String', 'Move down','Tag','variables2_3', ...
...
% new
uicontrol('Parent', tab4, 'Style', 'pushbutton', 'String', 'Move up (+)','Tag','variables2_v2', ...
...
uicontrol('Parent', tab4, 'Style', 'pushbutton', 'String', 'Move down (-)','Tag','variables2_v3', ...
    'HorizontalAlignment', 'left', 'Units','normal','BackgroundColor', 'green','Position', [0.51 0.69 0.1 0.03],'Callback',@movelims2);
%new end


uicontrol('Parent', tab4, 'Style', 'pushbutton', 'String', 'Update','Tag','variables2_4', ...
...
uicontrol('Parent', tab4, 'Style', 'pushbutton', 'String', 'Select Peak','Tag','variables2_5', ...
    'HorizontalAlignment', 'left', 'Units','normal','Position', [0.51 0.48 0.1 0.04]);  %machine 1 variable  selection of peaks button

uicontrol('Parent', tab4,'Style', 'popup','String', list_5 ,'Tag','variables2_6',...
    'Units','normal','Position', [0.51 0.44 0.1 0.04],'BackgroundColor', 'green','Callback', @selectpeaker); % %machine 2 variable  selection of peaks pop-up
...
uicontrol('Parent', tab4,'Style', 'popup','String', list_6 ,'Tag','variables2_13',...
    'Units','normal','Visible','off','Position', [0.51 0.3 0.1 0.04],'BackgroundColor', 'green','Callback', @selectpeakareas); %machine 2 variable  selection of spikes of peaks pop-up
...
uicontrol('Parent', tab4, 'Style', 'pushbutton', 'String', 'Select Spike','Tag','variables2_7', ...
    'HorizontalAlignment', 'left', 'Visible','off','Units','normal','Position', [0.51 0.2 0.1 0.04]); %machine 1 variable  selection of spikes of peaks button

uicontrol('Parent', tab4,'Style', 'popup','String', list_5 ,'Tag','variables2_8',...
    'Units','normal','Visible','off','Position', [0.51 0.16 0.1 0.04],'BackgroundColor', 'green','Callback', @selectspikes); %machine 2 variable  selection of spikes of peaks pop-up
...
try
    ...
    SecondMachine = evalin( 'base','SecondMachine');
    FirstMachine = evalin( 'base','FirstMachine');
...
if (sel_peak_1 > num_peaks_1) && (sel_peak_2 > num_peaks_2)
    ...
    for o=1:length(goter)
        ...
        try
            if sel_spike_k<difer
                ...
                if o == 1
                    titl = ['AREA = ' num2str(area_1) '; FORCE: Total = ' num2str(tot_fr) ' Mean: ' num2str(avg_fr) ' STD: ' num2str(std_fr) '; CONTACT TIME = ' num2str(diff_ti) ' BY SPIKE ' num2str(sel_spike_1) ' OF PEAK ' num2str(sel_peak_k) ' OF ' FirstMachine] ;% ' by Spike ' num2str(sel_spike_k) ' of ' list_11{sel_machine_1} ': ' list_21{sel_var_k} ': Peak ' num2str(sel_peak_k)])
                    uicontrol('Parent', pang_1, 'Style', 'pushbutton', 'String', upper(titl),'Tag','plut1', ...
                        'HorizontalAlignment', 'left', 'Units','normal','Visible','on','Position', [0.05 0.95 0.9 0.04]); %activity 1 button
                else
                    titl =['AREA = ' num2str(area_1) '; FORCE: Total = ' num2str(tot_fr) ' Mean: ' num2str(avg_fr) '; CONTACT TIME = ' num2str(diff_ti) ' BY SPIKE ' num2str(sel_spike_2) ' OF PEAK '  num2str(sel_peak_k) ' OF ' SecondMachine];% ' by Spike ' num2str(sel_spike_k) ' of ' list_11{sel_machine_2} ': ' list_21{sel_var_k} ': Peak ' num2str(sel_peak_k)])
                    uicontrol('Parent', pang_1, 'Style', 'pushbutton', 'String', upper(titl),'Tag','plut1', ...
                        'HorizontalAlignment', 'left', 'Units','normal','Visible','on','Position', [0.05 0.95 0.9 0.04]); %activity 1 button
                end
            ...
            elseif sel_spike_k==difer
                if o == 1
                    plot_areas(sel_var_k,res,sel_spike_k,sel_peak_k,rester,FirstMachine,fig_1,fig_2)
                else
                    plot_areas(sel_var_k,res_2,sel_spike_k,sel_peak_k,rester_2,SecondMachine,fig_1,fig_2)
                end
            end
        catch
        end
    ...
    for o = 1:length(gote)
        ...
        if sel_peak_k>length(dat)
            ...
            if o == 1
                FirstMachines=['ALL PEAKS OF ' FirstMachine];%' list_21{sel_var_1} ' of ' list_11{sel_machine_1}];
                SecondMachines=['ALL PEAKS OF '  FirstMachine]; % ' list_21{sel_var_2} ' of ' list_11{sel_machine_1}];
            else
                FirstMachines=['ALL PEAKS OF '  SecondMachine];% list_21{sel_var_1} ' of ' list_11{sel_machine_2}];
                SecondMachines=['ALL PEAKS OF '  SecondMachine];% list_21{sel_var_2} ' of ' list_11{sel_machine_2}];
            end
            dat_1 = res{sel_var_1};
            dat_2 = res_2{sel_var_2};
            if sel_peak_1>length(dat_1) && sel_peak_2>length(dat_2)
                
                editPCAdata(speaked_1,speaked_2,sb,sum_1,sum_12,mean_1,mean_12,FirstMachines,SecondMachines,1,fig_1,fig_2)
                return
            else
                if o ==2
                ...
                editPCAdata(speaked_1,speaked_2,sb,sum_1,sum_12,mean_1,mean_12,FirstMachines,SecondMachines,0,fig_1,fig_2)
            end
        else
        end
    end
...
pan_1 = uipanel('Parent',tab60,'Tag','resultpres_1','Position',[0.11 0.01 0.88 0.96],'Visible','on'); % machine 1 preview figure
...
plots_areas(s_spike_2,s_spike_3,sel_var_1,sel_var_2,res_1,res_2,sel_spike_1,sel_spike_2,sel_peak_1,sel_peak_2,rester_1,rester_2,machinename_1,machinename_2,pan_1,type,caser)
...
pan_1 = uipanel('Parent',tab60,'Tag','resultpres_1','Position',[0.01 0.01 0.98 0.98],'Visible','on'); % machine 1 preview figure
...
function  plots_areas(a1,b1,sel_var_1,sel_var_2,res_1,res_2,sel_spike_1,sel_spike_2,sel_peak_1,sel_peak_2,rester_1,rester_2,machinename_1,machinename_2,fig,type,caser)
...
for i=1:cula
    ...
    elseif type ==5 && i==1
        fi1 = uipanel('Parent',fig,'Tag','graphtpres_1','Position',[0.02 0.52 0.99 0.47],'Visible','on'); % machine 1 preview figure
        fi2 = uipanel('Parent',fig,'Tag','graphtpres_2','Position',[0.02 0.02 0.99 0.47],'Visible','on'); % machine 1 preview figure
...
%%
function  plot_areas(sel_var_1,res,sel_spike_1,sel_peak_1,rester,machinename,fig_1,fig_2)
if isempty(get(fig_1, 'Children'))
    fig = fig_1;
else
    fig = fig_2;
...
for i=1:cula
    ...
    try
        ...
        elseif caser == 4 || caser == 5 || caser == 6
            dut_1=[];
            dout_1=[];
            daout_1=[];
            dut_2=[];
...
function editPCAdata(speaked_1,speaked_2,sb,sum_1,sum_2,mean_1,mean_2,FirstMachine,SecondMachine,state,fig_1,fig_2)
% FirstMachine_1 = ['Overall Total = ' num2str(sum_1)];
% FirstMachine_2 = ['Overall Mean = ' num2str(mean_1)];
% SecondMachine_1 = ['Overall Total = ' num2str(sum_2)];
% SecondMachine_2 = ['Overall Mean = ' num2str(mean_2)];
...
a = uicontrol('Parent', tab1, 'Style', 'pushbutton', 'String', FirstMachine,'Tag','Transpose', ...
    'HorizontalAlignment', 'left','Units', 'normal', 'Position', [0.02  0.9 0.47 0.05]);

% a_1 = uicontrol('Parent', tab1, 'Style', 'pushbutton', 'String', FirstMachine_1,'Tag','Transpose1', ...
%     'HorizontalAlignment', 'left','Units', 'normal','Position', [0.02  0.05 0.47 0.04]);

% a_11 = uicontrol('Parent', tab1, 'Style', 'pushbutton', 'String', FirstMachine_2,'Tag','Transpose11', ...
%     'HorizontalAlignment', 'left','Units', 'normal', 'Position', [0.02  0.11 0.47 0.04]);

a1 = uicontrol('Parent', tab1, 'Style', 'pushbutton', 'String', 'Second Machine','Tag','Deletecolrow', ...
    'HorizontalAlignment', 'left','Units', 'normal', 'Position', [0.51  0.9 0.47 0.05]);

% a1_1 = uicontrol('Parent', tab1, 'Style', 'pushbutton', 'String', SecondMachine_1,'Tag','Transpose1_1', ...
%     'HorizontalAlignment', 'left','Units', 'normal','Position', [0.51  0.05 0.47 0.04]);
%
% a1_11 = uicontrol('Parent', tab1, 'Style', 'pushbutton', 'String', SecondMachine_2,'Tag','Transpose1_11', ...
%     'HorizontalAlignment', 'left','Units', 'normal', 'Position', [0.51  0.1 0.47 0.04]);
%
...
ED=  [1,10,20,30,40,50,72.5,75,77.5,80,82.5,85,86,88,90,91,92,93,94,94.5,95,95.5,96,96.25,96.5,96.7,96.85,97,97.25,97.5,97.75,98,98.25,98.5,99,99.2,99.5,99.55,99.7,99.9];
...
hf =hf(1);
% fh=findobj('Tag',strcat('variables',tag1(end-2),'_6'));
% fh = fh(1);
...
dat_machine_1 = evalin('base',strcat('dat_machine_',tag1));
sel_machine_1 = evalin('base',strcat('sel_machine_',tag1));
sel_machine_1(regexp(sel_machine_1 ,'_'))=' ';
sel_machine_1 = upper(sel_machine_1);
...
[res,rest,rester,trap_area,machine,timer,time_indices,ind_peak,num_ind_peak,region_3]= compare(dat_machine_1,lim_stater,sel_var_1);
[num_peaks_1,~,t_errorr,t_errorr_1,data_ind] =  eq_sel_var(sel_var_1,list_2,trap_area,res,machine);
...
if strcmp(tag1,'1')
    machine_state = 'FIRST MACHINE';
else
    machine_state = 'SECOND MACHINE';
end
...
assignin('base',strcat('num_peaks_',tag1),num_peaks_1);
plot_readings(sel_var_1,list_2,list_1,timer,sel_machine_1,data_ind,time_indices,ind_peak,num_ind_peak,machine_state,pan);
end


function [num_peaks_1,num_peaks_2,t_errorr,t_errorr_1,data_ind] = eq_sel_var(sel_var_1,list_2,trap_area,res,machine)
...
function [res,rest,rester,total,machine,timer,time_indices,ind_peak,num_ind_peak,region_3]= compare(brand,lim_stater,sel_var_1)
% get areas covered
        ...
        while (~success)
            try
                ...
                if val > 20
                    ...
                    warndlg('Current data has an issue, please check and resolve in database','Cannot Continue Error');
                    return
                end
            end
        end
...
con_out_groups = [con_in_groups(1);con_in_groups(diff(con_in_groups)>10);con_in_groups(end)];

expec = 2000 ;%round(length(data_ind)/50);
...
%con_out_groups = [con_in_groups(1);con_in_groups(diff(con_in_groups)>10);con_in_groups(end)];
%distinct_sessions = con_out_groups(diff(con_out_groups)>2000);
%disce = [distinct_sessions;con_in_groups(end)];
%assignin('base','ind_peaks',con_in_groups);
...
for i =1:length(bor)
    ...
    roter_1= [con_in_groupa(1);con_in_groupa(diff(con_in_groupa)>10);con_in_groupa(end)];
    try
        try
...
function  plot_readings(sel_var_i,list_2,list_1,timer,sel_machine_i,data_ind,time_indices,ind_peak,num_ind_peak,machine_state,pan)
%data_ind_1 =
tag1 = get(pan,'Tag');
...
% titled_1  = [list_2{sel_var_i} ' ' 'OF ' sel_machine_i];
